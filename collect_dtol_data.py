...
def main():
    ...
    raw_data = requests.get(
        f"https://www.ebi.ac.uk/ena/portal/api/filereport?accession={STUDY_ID}&result=read_run"
        f"&fields={','.join(experiment_fields)}&format=json&limit=0").json()
    # Collect all assemblies
    assemblies = requests.get(
        f"https://www.ebi.ac.uk/ena/portal/api/filereport?accession={STUDY_ID}&result=assembly"
    ...
    for sample_id, record in SAMPLES.items():
        if 'sample derived from' in record['characteristics']:
            ...
            if host_sample_id not in SAMPLES and host_sample_id not in additional_samples:
    ...
    if records_type is None:
        ...
        for sample_id, data in data_records.items():
            if sample_id not in SAMPLES:
                SAMPLES[sample_id] = requests.get(f"https://www.ebi.ac.uk/biosamples/samples/{sample_id}").json()
