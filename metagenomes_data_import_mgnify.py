...
ASG_URL = "https://portal.aquaticsymbiosisgenomics.org/api/root_organisms"
...
def main():
    ...
    data = list()
    # First request
    response = requests.post(f"{ASG_URL}?offset={offset}&limit=10").json()
    while offset + 10 <= response['count']:
        response = requests.post(f"{ASG_URL}?offset={offset}&limit=10").json()
        data.extend(response['rootSamples']['hits']['hits'])
        offset += 10
...
def collect_analyses_ids(record):
    ...
    for assembly_type in ["assemblies", "metagenomes_assemblies",
        ...
        if assembly_type in record and len(record[assembly_type]) > 0:
            for assmbl in record[assembly_type]:
                ...
                project_id = root.find("ASSEMBLY").find("STUDY_REF").find(
                ...
                response = requests.get(
                    ...
                    f"format=json&download=true&limit=0").json()
                for erz in response:
